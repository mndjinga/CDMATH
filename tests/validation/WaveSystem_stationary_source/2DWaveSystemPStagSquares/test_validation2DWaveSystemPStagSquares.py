...
def test_validation2DWaveSystemPStag_squares(scaling):
    ...
    for nx in meshList:
        ...
        error_p_tab[i], error_u_tab[i], mesh_size_tab[i], t_final[i], ndt_final[i], max_vel[i], diag_data_press[i], diag_data_vel[i], time_tab[i], cond_number[i] =WaveSystemPStag.solve(my_mesh,str(nx)+'x'+str(nx),resolution,scaling,meshType,testColor,cfl,True)
    ...
    plt.savefig(mesh_name+'_Pressure_2DWaveSystemSquaresPStag_'+"scaling"+str(scaling)+"_PlotOverDiagonalLine.png")
    plt.close()
    ...
    plt.savefig(mesh_name+"_2DWaveSystemSquarePStag_"+"scaling"+str(scaling)+"_TimeSteps.png")
