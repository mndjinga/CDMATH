...
def test_validation2DWaveSystemSquaresFV():
    ...
    for nx in meshList:
        ...
        error_p_tab[i], error_u_tab[i], mesh_size_tab[i], t_final[i], ndt_final[i], max_vel[i], diag_data_press[i], diag_data_vel[i], time_tab[i] =WaveSystemFV.solve(my_mesh,str(nx)+'x'+str(nx), mesh_name, resolution)
        i=i+1
